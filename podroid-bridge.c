...
int main(int argc, char *argv[]) {
     ...
     *
     * OPTIONAL: skip this for boots with menus (GRUB, UEFI) where an auto-Enter
     * would bypass the choice. Triggered by a third argument "no-cr". */
    int send_cr = 1;
    if (argc >= 4 && strcmp(argv[3], "no-cr") == 0) {
        send_cr = 0;
    }
    if (send_cr) {
